...
const CPUScheduler = ({ onBack }) => {
    ...
    return (
        <div className="flex h-screen bg-zinc-950 text-zinc-100 font-sans overflow-hidden selection:bg-indigo-500/30 w-full">
            ...
            <motion.aside
                ...
                animate={{
                    ...
                    filter: isRunning ? 'blur(2px) grayscale(50%)' : 'none',
                    pointerEvents: isRunning ? 'none' : 'auto'
                }}
                transition={{ duration: 0.5 }}
                className="w-96 bg-zinc-900/80 backdrop-blur-xl border-r border-white/5 flex flex-col z-20 shadow-2xl relative overflow-hidden"
            >
                ...
                <div className="p-6 border-b border-white/5 relative z-10">
                    <button
                        onClick={onBack}
                        className="flex items-center gap-2 px-4 py-2.5 mb-4 w-full text-sm text-zinc-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors"
                    >
            ...
            <main className="flex-1 flex flex-col bg-zinc-950 relative overflow-hidden">
                ...
                <header className="h-24 flex items-center justify-between px-10 z-10 relative">
                    <div className="flex items-center gap-6">
                        <div>
                            <h2 className="text-2xl font-bold text-white tracking-tight">{algorithm}</h2>
                            <p className="text-sm text-zinc-500 font-medium tracking-wide">Ready to optimize</p>
                        </div>
                        ...
                        <AnimatePresence>
                            {status === 'running' && (
                                <motion.div
                                    ...
                                    className="flex items-center gap-2 px-4 py-1.5 bg-cyan-500/10 border border-cyan-500/20 rounded-full shadow-[0_0_10px_rgba(34,211,238,0.1)]"
                                >
                                    <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse"></span>
                                    <span className="text-xs font-bold text-cyan-300 tracking-wider">EXECUTING</span>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
                    ...
                    {/* Unified Controls */}
                    <div className="flex items-center gap-6 bg-zinc-900/60 backdrop-blur-md p-2 rounded-2xl border border-white/5 shadow-xl">
                        {status !== 'idle' && (
                            <div className="flex items-center gap-4 px-4 border-r border-white/5 pr-6">
                        ...
                        <AnimatePresence mode="wait">
                            {status === 'idle' ? (
                                <motion.button
                                    ...
                                    className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-600/20 active:scale-95 transition-all flex items-center gap-2"
                                >
                                    <Play size={18} fill="currentColor" /> SIMULATE
                                </motion.button>
                            ) : (
                                <div className="flex items-center gap-3" key="controls">
                                    {status !== 'finished' && (
                                        <>
                                            <motion.button
                                            ...
                                            <motion.button
                                                ...
                                                whileTap={{ scale: 0.95 }}
                                                onClick={handleSkip}
                                                className="h-10 px-4 flex items-center justify-center bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 font-bold text-xs rounded-xl border border-cyan-500/20 transition-all shadow-md gap-2"
                                                title="Instant Result"
                                            >
                                                <SkipForward size={14} fill="currentColor" /> FINISH NOW
                                            </motion.button>
                                        </>
                                    )}
                                    ...
                                    <motion.button
                                        ...
                                        animate={{ opacity: 1, scale: 1 }}
                                        onClick={handleReset}
                                        className={`px-5 py-2.5 ${status === 'finished' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20'} rounded-xl font-bold text-xs transition-all flex items-center gap-2`}
                                    >
                                        <RotateCcw size={16} /> {status === 'finished' ? 'RESET' : 'STOP'}
                                    </motion.button>
                                </div>
                            )}
                        </AnimatePresence>
